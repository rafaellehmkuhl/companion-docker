...
class PlatformManager:
    ...
    def start_navigator_process(
        self, endpoint: Endpoint, firmware_path: pathlib.Path, log_path: pathlib.Path, storage_path: pathlib.Path
    ) -> None:
        self.launch_args = {
            "endpoint": endpoint,
        ...
        self._running_platform = Platform.Navigator
    ...
    def running_ardupilot_processes(self) -> List[psutil.Process]:
        ...
        def is_ardupilot_process(process: psutil.Process) -> bool:
            ...
            return self._running_platform.value in " ".join(process.cmdline())
    ...
    def ardupilot_process_not_running(self) -> bool:
        return (self.ardupilot_subprocess is not None and self.ardupilot_subprocess.poll() is not None) or len(
            self.running_ardupilot_processes()
        ) != 0
    ...
    async def auto_restart_ardupilot_process(self) -> None:
        ...
        while True:
            ...
            try:
                if current_platform in [Platform.Navigator, Platform.NavigatorR3]:
